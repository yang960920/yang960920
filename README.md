![header](https://capsule-render.vercel.app/api?type=waving&color=timeGradient&height=250&section=header&text=Hello,%20I'm%20Hyunjun!&fontSize=70&fontAlignY=38&desc=Customer-Centric%20Backend%20Developer&descAlignY=55&descAlign=62)

<div align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.herokuapp.com?font=Pretendard&weight=700&size=22&pause=1000&color=2196F3&center=true&vCenter=true&width=600&lines=고객의+시선에서+비즈니스를+이해하는+개발자;코드 한 줄로+업무의+비효율을+걷어냅니다;소통과+공감으로+팀을+이끄는+프로젝트+리더" alt="Typing SVG" />
  </a>
</div>

<br>

## 🙋‍♂️ About Me
**"고객 만족 전략가에서, 실질적인 문제를 코드로 해결하는 백엔드 개발자로!"**

과거 글로벌 OTT 기업과 F&B 브랜드에서 고객 경험(CX)을 개선하고 시스템을 효율화했던 경험을 바탕으로, 이제는 **코드를 통해 비즈니스 로직과 사내 프로세스를 혁신**하고 있습니다. 단순히 돌아가는 코드를 넘어서, '사용자가 진짜 필요로 하는 것'이 무엇인지 고민합니다.

- 🚀 **실무 밀착형 문제 해결:** Google Apps Script를 활용해 배차 일지, 자산 관리, 일일 업무 보고 등 사내 비효율을 걷어내는 맞춤형 웹 어플리케이션을 기획하고 개발합니다.
- 💡 **리더십 & 커뮤니케이션:** Java 백엔드 팀 프로젝트 리더로서, 위기 상황을 투명한 소통과 멘토링으로 극복하여 우수상을 이끌어낸 경험이 있습니다.
- 🌐 **유연한 배포와 인프라:** Vercel 환경에서의 다중 페이지 커뮤니티 및 기업 홈페이지 구축 경험을 통해 프론트엔드 배포와 DB 연동 등 폭넓은 시야를 갖추고 있습니다.

---

## 🛠️ Tech Stack & Tools

### 💻 Backend & Language
<a href="https://www.java.com/" target="_blank"><img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=java&logoColor=white"></a>
<a href="https://developers.google.com/apps-script" target="_blank"><img src="https://img.shields.io/badge/Google Apps Script-4285F4?style=for-the-badge&logo=google&logoColor=white"></a>

### 🌍 Frontend & Deployment
<a href="https://developer.mozilla.org/ko/docs/Web/HTML" target="_blank"><img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"></a>
<a href="https://developer.mozilla.org/ko/docs/Web/CSS" target="_blank"><img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"></a>
<a href="https://developer.mozilla.org/ko/docs/Web/JavaScript" target="_blank"><img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"></a>
<a href="https://vercel.com/" target="_blank"><img src="https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white"></a>

### 🗄️ Database
<a href="https://neon.tech/" target="_blank"><img src="https://img.shields.io/badge/Neon DB-00E599?style=for-the-badge&logo=neon&logoColor=black"></a>

### 🤝 Collaboration
<a href="https://git-scm.com/" target="_blank"><img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"></a>
<a href="https://github.com/" target="_blank"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"></a>

---

## 📊 Tech Proficiency & Stats

### 🔥 Skill Proficiency
| Tech Stack | Level | Description |
| :--- | :---: | :--- |
| **Java** | 🟢🟢🟢🟢⚪ | 객체 지향 기반의 백엔드 아키텍처 설계 및 비즈니스 로직 구현 |
| **Google Apps Script** | 🟢🟢🟢🟢🟢 | 사내 인프라(배차, 자산, 업무 일지) 자동화 및 맞춤형 웹앱 개발 |
| **JavaScript** | 🟢🟢🟢⚪⚪ | 다중 페이지 웹사이트의 동적 기능 구현 및 DOM 제어 |
| **HTML / CSS** | 🟢🟢🟢⚪⚪ | 웹 표준을 준수하는 구조 설계 및 Vercel 환경 배포 |
| **Database (Neon)** | 🟢🟢⚪⚪⚪ | RDBMS 기본 설계 및 쿼리 최적화 |

<br>

### 🏆 Most Used Languages
<div align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=yang960920&layout=donut&theme=tokyonight&hide_border=true&card_width=450" alt="Top Languages" />
</div>
